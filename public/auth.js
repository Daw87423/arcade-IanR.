...
(function () {
  ...
  let hbTimer = null, lastMsgTs = 0, lastNewsTs = 0, firstPoll = true;
  ...
  const Arcade = {
    ...
    startHeartbeat() {
      ...
      const tick = async () => {
        ...
        const news = await this.api('news', { token: t, action: 'list' });
        if (news.ok && news.data && news.data.feed) {
          const posts = news.data.feed.slice().sort((a, b) => (a.ts || 0) - (b.ts || 0));
          if (!firstPoll) { for (const p of posts) { if ((p.ts || 0) > lastNewsTs) showToast('Fil d’actualités', p.text); } }
          for (const p of posts) lastNewsTs = Math.max(lastNewsTs, p.ts || 0);
        }
        firstPoll = false;
      };
      tick(); hbTimer = setInterval(tick, 12000);
    },
    stopHeartbeat() { if (hbTimer) { clearInterval(hbTimer); hbTimer = null; } firstPoll = true; lastMsgTs = 0; lastNewsTs = 0; },
  };
